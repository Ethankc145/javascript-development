...
const charTypeSelectionMenu = `Please select character type: \n1) Letters only \n2) Numbers only\n3) Numbers and Letters only\n4) Special Characters Only\n5) Mixed\n6) Exit\n`
let customLength = "";
let excludedChars = ""
  ...
  if (excludedChars) {
    ...
    return password.replace(
      ...
      passTypeWithExclusions.charAt(
        Math.floor(Math.random() * passTypeWithExclusions)
      )
    )
  }
...
switch (lengthChoice) {
  ...
  case "3":
    console.log(
      "\n" + generatePassword(specialChars, checkCharType(characterChoice))
    );
    break;
  }
